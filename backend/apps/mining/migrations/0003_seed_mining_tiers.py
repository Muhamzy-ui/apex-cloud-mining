...
def create_mining_tiers(apps, schema_editor):
    ...
    TIERS = [
        ...
        {'tier_number': 5, 'name': 'Plan 5', 'price_usd': 435.99, 'earn_per_24h_usd': 699.00, 'duration_days': 30, 'withdrawal_fee_usd': 25.00},
    ]

    for td in TIERS:
        MiningTier.objects.update_or_create(
            tier_number=td['tier_number'],
            defaults=td
        )

def reverse_create_mining_tiers(apps, schema_editor):
    MiningTier = apps.get_model('mining', 'MiningTier')
    MiningTier.objects.all().delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_mining_tiers, reverse_create_mining_tiers),
    ]
